        ...
        for (var i=0; i<gd.length; i++) {
            ...
            if (this.showSlice[i]) {
                ...
                if (this.showDataLabels && gd[i][2]*100 >= this.dataLabelThreshold) {
                    ...
                    var fact = (this._radius*1.5) * this.dataLabelPositionFactor + this.sliceMargin + this.dataLabelNudge;
                
                    var x = this._center[0] + Math.cos(avgang) * fact + this.canvas._offsets.left;
                    var y = this._center[1] + Math.sin(avgang) * fact + this.canvas._offsets.top;
                
                    var labelelem = $('<div class="jqplot-pie-series jqplot-data-label" style="color:white;position:absolute;font-size:10px">' + label + '</div>').insertBefore(plot.eventCanvas._elem);
